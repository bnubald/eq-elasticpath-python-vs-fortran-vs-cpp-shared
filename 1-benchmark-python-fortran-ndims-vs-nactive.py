...
A, B = 20, 20
xn = 2500
relpct = np.linspace(0.01, 1, num=A)
xm = np.linspace(A, 1000, num=B).astype(int)
...
for i, c in enumerate(xm):
    print(i, '--', A )
    for j, r in enumerate(relpct):
        X, y = gen_linear(n_observations=xn, n_dim=c, n_relevent=int(r*c), bias=0.0, noise=0.0, random_seed=0)
...
ax1.xaxis.set_label_text("Dimensions")
ax1.yaxis.set_label_text("Observations")
